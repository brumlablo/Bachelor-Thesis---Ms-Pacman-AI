...
Výňatek z kódu \ref{code:abaction} ukazuje metodu \texttt{getAction} u agenta \texttt{AlphaBetaAgent}\footnote{Komentáře v kódu jsou psány anglicky, aby navazovaly na framework. Výňatek má komentáře přeloženy.}.

\begin{python}[caption={získání akce u \texttt{AlphaBetaAgent}},label={code:abaction}]
...
\begin{python}[caption={Konstruktor \texttt{ValueIterationAgent}},label={code:valagent}]
...
for k in range(iterations):
   ...
   for state in self.mdp.getStates():  # pro kazdy stav modelu (1 pole mrizky)
      ...
      fut\label{navrh:qlearnagent}ureValues[state] = value
...
\begin{center}
  ...
  \caption{Screenshot počáteční modelu gridworld problému \texttt{BridgeGrid} pro Value Iteration agenta (Q-Learning agent toto ohodnocení odměn stavů nemá) ukazuje stavy a jejich odměnu. Agent je reprezentovaný modrou tečkou.}
  \label{img:bridgeGridDefault}
\end{center}
\end{figure}

Výsledné provedení chování agenta je velmi podobné jako u Q-Learning Agentů - viz výňatek z kódu \ref{code:qpolicy}, avšak lze pro optimalitu strategie provést až po provedení iterací V-hodnot (v kódu objektová proměnná \textit{values}) a neprovádí trénink (nevyužívá $\epsilon-greedy$), takže se vždy provede nejlepší možná akce stavu na základě odhadů Q-hodnot z modelu.

\section{Q-Learning agenti (\texttt{QLearningAgent},\texttt{PacmanQAgent)}}
Jak již bylo řešeno v \ref{navrh:qlearnagent}, tento agent si musí model odhadnout sám na základě získaných odměn při výběru akcí na základě \textit{exporation/exploitation ratio}. Tento poměr je dán instanční proměnnou \texttt{epsilon} a předán funkci \texttt{flipCoin}, jež určí finální rozhodnutí výběru akce. Nastavení defaultních parametrů:
\newline
\texttt{QLearningAgent} alpha=1.0, epsilon=0.05, gamma=0.8, numTraining = 10
\newline
\texttt{PacmanQAgent)} epsilon = 0.05, gamma = 0.8, alpha = 0.2, numTraining = 0

\begin{python}[caption={Vyhodnocení strategie pro stav během tréninku Q-Learning agentů},label={code:qpolicy}]
def getAction(self, state):
   ...
   if flipCoin(self.epsilon):
      ...
      return random.choice(actions) # nahodnoa akci pro stav (\textit{exploration})
   else:
      return self.getPolicy(state) # nejlepsi mozna akce na zaklade odhadu Q-hodnot (\textit{exploitation})
\end{python}


\section{Aproximační Q-Learning agent (ApproximateQAgent)}
noactionfn, numTraining=100, epsilon=0.5, alpha=0.5, gamma=1
\newline
