...
Agenty lze srovnat nejlépe podle průměrného dosaženého herního skóre v době vykonávání optimální strategie (trénink se nepočítá). U učících agentů bude též pozorován počet epizod, za jak dlouho jsou schopni průměrně uspět alespoň ve dvou třetinách z celkového počtu her. Touto mírou jsou následně prohlášeni za \textit{úspěšné}.
...
\begin{figure}[!htbp]
\begin{center}
  \includegraphics[height=150px,width=400px]{charts/smallGrid_score.png}
  \includegraphics[height=150px,width=400px]{charts/smallGrid_winrate.png}
...
\begin{figure}[!htbp]
\begin{center}
  \includegraphics[height=150px]{charts/smallgrid_iter_q.png}
  \includegraphics[height=150px]{charts/smallgrid_iter_approxq.png}
...
\begin{figure}[!htbp]
\begin{center}
	\includegraphics[height=150px]{charts/betterext_winrate.png}
  	\caption{Stagnace výsledků vektoru vlastností Aproximačního Q-Learningu. }
  \label{chart:episodes}
\end{center}
\end{figure}

Kromě předchozí mapy bylo zkoumáno chování agentů  i na mapě \texttt{trappedClassic}. Během experimentů se ukázalo, že je tato mapa zajímavá především kvůli sebevražedným pokusům základních agentů \texttt{MinimaxAgent} a \texttt{AlphaBetaAgent}. Agenti si pesimisticky spočtou své naděje na přežití a raději se zabijí a skončí tak hru, ačkoliv jejich nepřítel stochastický. Naproti tomu \texttt{ExpectimaxAgent} a pokročilí agenti počítají s náhodou. Tato mapa posloužila jako příklad velkého rozdílu vyhodnocení chování základních agentů.

\section{Zatěžkávací zkouška na mapě \texttt{trickyClassic}}

Tato mapa slouží především jako zatěžkávací test pro agenty zvládající její výpočetní náročnost (problémy byly zmíněny v úvodní části této kapitoly). Jak je vidět na grafech \ref{chart:tc} \texttt{ReflexAgent} již nedokázal poradit s otevřenými plochami mapy a větším množstvím nepřátel této velké mapy. \texttt{ExpectimaxAgent} dokázal vyhrát alespoň ve 30 \% případů a udržet si již poměrně vysoké průměrné skóre nad 1000 bodů. Experimenty tedy ukázaly, že kvalita ohodnocovací funkce tohoto agenta je nedostačující pro takto náročnou mapu. Agent se také často zbytečně zasekával v rozích. To řeší až \texttt{ApproximateQAgent}, který se své optimální chování postupně naučil už po cca 100 epizodách tréninku. Tabulka Q-Learningového agenta tuto mapu nedokázala pojmout a agent nedokázal uspět ani po 3000 epizodách tréninku, proto není zanesen do grafu. \texttt{ApproximateQAgent} zde jasně prokázal, že dokáže rychle získat  určitý level inteligence, oproti němu \texttt{PacmanQAgent} těžce zaostává mnohonásobným počtem iterací nutných pro svou úspěšnost a neúnosností své tabulky Q-hodnot\footnote{Postupně rostoucí tabulka hodnot párů (stav,akce) Q-learning agenta si exponenciálně nárokuje víc a víc vypočetního výkonu až do té míry, že přístup do této tabulky není vypočetně použitelný pro větší mapu.}.

\begin{figure}[!htbp]
\begin{center}
  \includegraphics[height=150px,width=400px]{charts/trickyClassic_score.png}
  \includegraphics[height=150px,width=400px]{charts/trickyClassic_winrate.png}
