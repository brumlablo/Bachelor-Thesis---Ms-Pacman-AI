...
\end{itemize}
Každý agent dědí z objektu Agent základní metody a proměné (např. proměnné definující základní směry pohybu; metoda getAction, jež zavazuje agenta přijmout objekt GameSate a vrátit na něj reakci v podobě akce.), avšak implementace vlastního chování je již definována v konkrétním objektu (jako override). Např. AlphaBetaAgent implementuje metodu getAction jako algoritmus Alfa-Beta řezy atp.
...
Jako ukázku lze uvést pseudokód algoritmu Expectimax, který je z hlediska porovnání s pokročilými metodami umělé inteligence nejzajímavější.
