...
\begin{algorithm}
\caption{\textbf{Value Iteration} - pseudokód I.část}
\label{alg:valiter1}
\begin{algorithmic}[1]

\Require model, koeficient gamma, počet episod, počet iterací,presnost
\Function{ValueIteration}{model,gamma,maxEpisod,maxIteraci,presnost}
	\For{i = 0, i $<$ maxIteraci,i++}
		noveVHodnoty = vHodnoty \Comment{ $V_{k}$}
		\For{stav $\in$ model.stavy}
			\If{$\text{\Call{model.terminalniUzel}{stav}}$}
				\State \Return noveVHodnoty[stav] = 0
			\EndIf
			\State hodnota = $-\infty$ \Comment{ $V_{k+1}$}
			\ForAll{pripustne akce z modelu}
				\State odhadQ = $\text{\Call{getQValue}{stav,akce}}$
				\If{odhadQ $>$ hodnota} \Comment{největší možná Q-hodnota}
					\State hodnota = odhadQ
				\EndIf
			\EndFor
			\State noveVHodnoty[stav] = hodnota
		\EndFor
		\State \Return vHodnoty = noveVHodnoty
		\If{$\left| \text{noveVHodnoty[stav] - noveVHodnoty[stav]}\right| < $ presnost}
			\State \Return noveVhodnoty[stav]
		\EndIf
	\EndFor
	\State \Return noveVhodnoty[stav]
\EndFunction
...
\Require stav, vektor V-Hodnot stavů
\Ensure V-Hodnoty(stav)
\Function{getValue}{stav}
    \State \Return vHodnoty[stav]
\EndFunction

\algrule

\Require model,stav,akce,koeficient gamma, vektor V-Hodnot stavů
\Ensure Q-Hodnota(stav,akce)
\Function{getQValue}{stav,akce}
    \State qHodnota = 0.0
    \ForAll{naslednici z modelu}	
		\State p = $\text{\Call{model.prechodovaFunkce}{stav,akce,naslednik}}$ \Comment{pravděpodobnost}
		\State r = $\text{\Call{model.odmenovaFunkce}{stav,akce,naslednik}}$ \Comment{odměna}
		\State qHodnota += p * ( r + gamma * $\text{\Call{getValue}{naslednik}})$
    \EndFor
...
\begin{algorithm}
\caption{\textbf{Value Iteration } - pseudokód II. část}
\label{alg:valiter2}
\begin{algorithmic}[1]
\algrestore{bkbreak}
\Require model, stav hry
\Ensure nejlepší možná akce z daného stavu ($\pi$(stav))
\Function{getPolicy}{model,stav}
	\If{terminalni uzel}
		\State \Return NULL\Comment{terminalní uzel nemá žádnou možnou akci k provedení; z modelu}
	\EndIf
	\State hodnota = $-\infty$
    \ForAll{pripustne akce z modelu}
		\State tmp = $\text{\Call{getQValue}{stav,akce}}$
		\If{tmp $>$ hodnota} \Comment{největší možná Q-hodnota}
		    \State hodnota = tmp
			\State policy = akce
    	\EndIf
    \EndFor 
