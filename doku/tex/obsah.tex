...
\end{displaymath}
\item opakuj předchozí krok dokud vektor $V_k$ nekonverguje (konvergence zapřičiňuje \gamma, což zaručuje optimálnost metody)
\end{enumerate}
...
Po nastudování teorie je důležité jasně poupravit cíle práce: budou se porovnávat nejen metody Minimax a Alfa-Beta Řezy s učicím agentem, ale především také Expectimax, který bere v potaz stochastičnost akcí duchů.
...
Pro Expectimax byla dále naimplementována vyhodnocovací funkce, která dosáhla lepších výsledků než funkce předchozí. Popis, co je lepší, proč, jak by se to dále dalo vylepšit - například pomocí algoritmů pro prohledávání stavového prostoru.
...
\section{Zvolené prostředky pro experimentování}
Nejdříve bylo nutné si zvolit programovací jazyk, který by zvládl výpočetní náročnost problematiky vůči vykreslování grafického dema. Zpočátku byl zvolen jazyk Java díky její vysoké míře abstrakce vůči programátorskému myšlení. Nakonec však po sérii experimentování s jazykem se kvůli náročnosti samotné logiky hry vůči GUI a především kvůli efektivnímu zvládnutí rozgenerování všech možných stavů hry a dodržení matematických vzorců, jež byly stanoveny v předchozí teoretické kapitole přešlo k hledání náhradního řešení - například nalezení frameworku, jež řeší podobnou problematiku. Základem pro studii a experimentování s algoritmy nakonec posloužil pythonový framework od UC Berkeley, používaný v jejich výuce Umělé inteligence - ODKAZ. Hlavním důvodem zvolení frameworku bylo především zaměření práce na experimentování s algoritmy umělé inteligence, ne implemetace aplikace. Dalšími důvody je je použití jazyka Python 2.7.5, který též umožňuje objektový přístup, je dobře čitelný a nenáročný - nejen díky těmto kladům se dají dobře pochopit vlastnosti a schéma jednotlivých objektů frameworku.

\section{Schéma aplikace}
Nejdůležitější částí aplikace je objekt GameState reprezentující \textbf{stav hry}.
- objektový schéma framework
Detailní popis implementace frameworku lze pak najít v komentářích v kódu.

\subsection{Reprezentace stavu hry}
GameState popis a jak s ním pracovat - popis, co jde využít, jak
Popis výpočtu manhattonovských vzdáleností, proč toto využívat
MDP (akce,stav)
