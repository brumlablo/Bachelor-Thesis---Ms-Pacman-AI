...
Z vypočetních (nutnost stanovit model) a názorných důvodů je tento agent zobrazen výhradně formou \textbf{gridworld} problémů (mřížky hodnot začínající se definovanými odměnami na obr. XYZ; spustitelné pomocí \texttt{gridworld.py}), kde jedno políčko po provedení metody ukazuje svou výslednou V-hodnotu, po zmáčknutí klávesy (např. "Q") své 4 Q-hodnoty pro každý směr/akci a následně provede stanovený počet epizod (agentovo provedení výsledné strategie). Gridworld problémů je hned několik, avšak nejzajímavější již zmíněný a defaultní BookGrid, DiscountGrid a BridgeGrid budou experimenálně zkoumány v následující kapitole.
\newline
